.PHONY: init venv deps dirs clean pytest coverage test mypy pylint ruff check build

.DEFAULT_GOAL := check
FILES_CHECK_MYPY := main.py
FILES_CHECK_ALL := $(FILES_CHECK_MYPY)
COVERAGE_TARGET := pr-policy-checker
PYTHON_VER := 3.12
...
venv:
	if which python$(PYTHON_VER); then \
		...
		PYTHON_BINARY=$$(pyenv prefix $(PYTHON_VER) | tail -1)/bin/python$(PYTHON_VER); \
	fi; \
